...
namespace Digital_Services_BD.Services
{
    public class CartOps : ICartOps
    {
        ...
        {
            var cart = string.IsNullOrEmpty(userId) ? null : await context.Carts.AsNoTracking()
        ...
        {
            quantity = Math.Clamp(quantity, 0, ProductConfig.MaxItemAllowedInCart);
            ...
            if (cart != null && productItem != null && productItem.IsActive && quantity > 0 && productItem.ProductStockCount?.Count > 0)
            {
                int countInBundle = 0;
                ...
                else //In cart there exists the product item already
                {
                    ...
                    if (cartItem != null)
                    {
                        cartItem.Quantity = (int)Math.Min((long)cartItem.Quantity + quantity, ProductConfig.MaxItemAllowedInCart);
        ...
        {
            var cartItem = await context.CartItems.FirstOrDefaultAsync(item => item.Id == cartItemId && item.CartId == cartId);
        ...
        public async Task<bool> EmptyCart(int cartId)
        {
            ...
            if (cartExists)
            {
                try
                {
                    var items = await context.CartItems.Where(item => item.CartId == cartId).ToListAsync();
                    var bundles = await context.CartProductItemBundles.Where(bundle => bundle.CartId == cartId).ToListAsync();
                    context.CartItems.RemoveRange(items);
                    context.CartProductItemBundles.RemoveRange(bundles);
                    await context.SaveChangesAsync();
                    return true;
                }
                catch (Exception e)
        ...
        public async Task<Cart> MergeCarts(int anonymousCartId, string userId)
        {
            try
            {
                        ...
                        .Include(c => c.CartProductItemBundles)
                            .ThenInclude(bundle => bundle.ProductItemBundle)
                                .ThenInclude(bundle => bundle.ProductItemBundleJoinProductItem)
                                    .ThenInclude(join => join.ProductItem)
                                        .ThenInclude(pi => pi.ProductStockCount)
                        .FirstOrDefaultAsync(c => c.Id == anonymousCartId && (c.UserId == null || c.UserId == userId));
                ...
                if (anonymousCart == null) return userCart ?? await CreateCart(userId);
                if (userCart == null) { anonymousCart.UserId = userId; await context.SaveChangesAsync(); return anonymousCart; }
                if (anonymousCart.Id == userCart.Id) return userCart;

                if (anonymousCart != null && userCart != null && anonymousCart.Id != userCart.Id)
                {
                    //Items
                    foreach (var item in anonymousCart.CartItems.ToList())
                    {
                        ...
                        if (userCart.CartItems.Any(cartItem => cartItem.ProductItemId == item.ProductItemId))
                        {
                            ...
                            cartItem.Quantity = (int)Math.Min((long)cartItem.Quantity + item.Quantity, ProductConfig.MaxItemAllowedInCart);
                    ...
                    //Bundle items
                    foreach (var bundle in anonymousCart.CartProductItemBundles.ToList())
                    {
                        ...
                        if (userCart.CartProductItemBundles.Any(bndl => bndl.ProductItemBundleId == bundle.ProductItemBundleId))
                        {
                            ...
                            userCartBundle.Quantity = (int)Math.Min((long)userCartBundle.Quantity + bundle.Quantity, ProductConfig.MaxItemAllowedInCart);
                            var minStock = bundle.ProductItemBundle.ProductItemBundleJoinProductItem
                                .Min(join => join.ProductItem.ProductStockCount.Count / Math.Max(1, join.ProductItemQuantity));
        ...
        public async Task<CartViewModel> GetCart(int? cartId)
        {
            ...
            if (cart != null)
            {
                var invalidItems = cart.CartItems.Where(item => CartPricing.GetPrice(item.ProductItem) == null).ToArray();
                foreach (var item in invalidItems) cart.CartItems.Remove(item);
                var invalidBundles = cart.CartProductItemBundles.Where(line => !CartPricing.IsValidBundle(line.ProductItemBundle)).ToArray();
                foreach (var line in invalidBundles) cart.CartProductItemBundles.Remove(line);
                if (invalidItems.Length + invalidBundles.Length > 0) await context.SaveChangesAsync();
                //Remove cartItems with 0 quantity/out of stock items
                string message = invalidItems.Length + invalidBundles.Length > 0 ? "Unavailable products were removed from your cart." : null;
                ...
                cartViewModel = new CartViewModel
                {
                    ...
                    CartItemBundlesViewModel = cart.CartProductItemBundles.Select(CartPricing.CreateBundle).ToList(),
                    PromoCode = null,
                    PriceCurrency = "BDT",
                    PromoCodeDiscount = 0,
                    TaxesAndFees = 0,
                ...
                {
                    var price = CartPricing.GetPrice(cartItem.ProductItem)!;
                    cartItem.Price = price.Price;
                    cartItem.Discount = price.Discount;
                ...
                cartViewModel.DiscountTotal = cart.CartItems.Sum(item => item.Discount * item.Quantity)
                    + cartViewModel.CartItemBundlesViewModel.Sum(bundle => bundle.BundleDiscount * bundle.Quantity)
                    + cartViewModel.PromoCodeDiscount;
            }
            else
            {
            ...
            int productItemBundleId, int quantity)
        {
            ...
            if (productItemBundle == null || !productItemBundle.IsActiveNow || !productItemBundle.ProductItemBundleJoinProductItem.Any()
                || productItemBundle.ProductItemBundleJoinProductItem.Any(item => item.ProductItemQuantity <= 0)) return null;
            quantity = Math.Clamp(quantity, 0, ProductConfig.MaxItemAllowedInCart);
            ...
            if (cart != null && productItemBundle != null && quantity > 0 && minStock > 0)
            {
                if (!await DoesProductItemBundleExistInCart(cart.Id, productItemBundle.Id))
                {
                    ...
                    {
                        int countInCartItems = cart.CartItems.Where(ci => ci.ProductItemId == join.ProductItemId).Sum(ci => ci.Quantity);
        ...
        public async Task<Cart> RemoveOutOfStockItems(int cartId)
        {
            try
            {
                ...
                if (cart == null) return null;
                return await RemoveOutOfStockItemsFromGivenCart(cart);
            }
            catch
            {
                return null;
